...
shooty_export:SetBackdropColor(TOOLTIP_DEFAULT_BACKGROUND_COLOR.r, TOOLTIP_DEFAULT_BACKGROUND_COLOR.g,
...
local class_cache = setmetatable({}, {
...
function sepgp_standings:BuildStandingsTable(raid_only)
  ...
  sepgp.alts = {}
  for i = 1, GetNumGuildMembers(1) do
    local name, rankName, _, _, class, _, note, officernote, _, _ = GetGuildRosterInfo(i)
    local ep = (sepgp:get_ep_v3(name, officernote) or 0)
    local gp = (sepgp:get_gp_v3(name, officernote) or sepgp.VARS.basegp)
  ...
  if (sepgp_groupbyclass) then
    table.sort(t, function(a, b)
      if (a[2] ~= b[2]) then
        ...
        return pr_sorter_standings(a, b)
      end
    end)
...
function sepgp_standings:OnTooltipUpdate()
  ...
  for i = 1, table.getn(t) do
    local name, class, armor_class, ep, gp, pr, rank = unpack(t[i])
