...
sepgp.VARS = {
  ...
  allinmsgp = "ALL IN - Mainspec GP",
  needmsgp = "NEED - Mainspec GP",
  minmsgp = "MIN - Mainspec GP",
  osgp = "Offspec GP",
  bankde = "Bank-D/E",
    ...
    if (sepgp_altspool) then
      ...
      if oldmain ~= nil then
        if main == nil or main ~= oldmain then
          ...
          self:defaultPrint(string.format(L["|cffff0000Manually modified %s\'s note. Previous main was %s|r"], name,
            oldmain))
        end
      end
    end
...
local sepgp_auto_gp_menu = {
  ...
  {
    text = L["Add NEED GP (15)"],
    func = function()
      local dialog = StaticPopup_FindVisible("SHOOTY_EPGP_AUTO_GEARPOINTS")
      if (dialog) then
        local data = dialog.data
        local player, off_price = data[sepgp.loot_index.player], 15
        sepgp:givename_gp((player == YOU and sepgp._playerName or player), off_price)
        sepgp:refreshPRTablets()
        data[sepgp.loot_index.action] = sepgp.VARS.needmsgp
        local update = data[sepgp.loot_index.update] ~= nil
        sepgp:addOrUpdateLoot(data, update)
        StaticPopup_Hide("SHOOTY_EPGP_AUTO_GEARPOINTS")
        sepgp_loot:Refresh()
  ...
  },
  {
    ...
    func = function()
      ...
      if (dialog) then
        local data = dialog.data
        local player, off_price = data[sepgp.loot_index.player], 5
        sepgp:givename_gp((player == YOU and sepgp._playerName or player), off_price)
        sepgp:refreshPRTablets()
        data[sepgp.loot_index.action] = sepgp.VARS.minmsgp
        local update = data[sepgp.loot_index.update] ~= nil
        sepgp:addOrUpdateLoot(data, update)
        StaticPopup_Hide("SHOOTY_EPGP_AUTO_GEARPOINTS")
        sepgp_loot:Refresh()
  ...
  },
  {
    ...
    func = function()
      ...
      if (dialog) then
        local data = dialog.data
        local player, off_price = data[sepgp.loot_index.player], 0
        sepgp:givename_gp((player == YOU and sepgp._playerName or player), off_price)
        sepgp:refreshPRTablets()
